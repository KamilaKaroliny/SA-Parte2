...
$rel_ano       = isset($_GET['ano']) ? (int)$_GET['ano'] : 0;
$rel_mes       = isset($_GET['mes']) ? (int)$_GET['mes'] : 0;

if ($maquinista_id <= 0 || $rel_ano <= 0 || $rel_mes <= 0) {
...
if (!$stmt_user) {
    die("Erro no SQL (usuarios): " . $mysqli->error);
}
...
if (!$stmt_rel) {
    die("Erro no SQL (relatórios): " . $mysqli->error);
}
