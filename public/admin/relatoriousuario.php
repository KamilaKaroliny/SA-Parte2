<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Relatorio</title>
    <link rel="stylesheet" href="../../style/style.css">
    <script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
    <script src="../../scripts/graficoJarbas.js"defer></script>
</head>
<body>
    <!---cabeçalho -->
    <div id="flex4">
        <div class="meio1">
            <a href="telaMaquinista.php">
                <img id="seta" src="../../assets/icons/seta.png" alt="seta">
            </a>
        </div>

        <div class="meio1">
            <img id="logo4" src="../../assets/icons/logoTremalize.png" alt="logo">
        </div>

        <div class="meio2">
            <a href="paginaInicial.php">
                <img id="casa1" src="../../assets/icons/casa.png" alt="casa">
            </a>
        </div>
    </div>

    <!-- informações ao lado da foto do maquinista -->
    <div id="flex10">
        <img id="foto" src="../../assets/images/jarbas.png" alt="Imagem Jarbas">
        <div class="dados">
            <span><strong>Nome:</strong>Jarbas Andrade</span><br>
            <span><strong>Telefone:</strong>  +55 (21) 97654-3210</span><br>
            <span><strong>Idade:</strong> 34 anos</span><br>
            <span><strong>ID:</strong> MSJ870NSHXU6</span><br>
            <span><strong>Tipo:</strong> Turismo</span>
        </div>
    </div>

    <!-- div com as imagens e informações como; velocidade média e quando já percorreu-->
    <div id="flex">
        <div class="quadradinho4">
            <img id="imgTelaRela" src="../../assets/icons/kmh.png" alt="Imagem chat">
            <H2>54 KM/H</H2>
        </div>

        <div class="quadradinho4">
            <img id="imgTelaRela" src="../../assets/icons/km.png" alt="Imagem relatorio">
            <H2>220.451 KM</H2>
        </div>
    </div>

    <!-- seção do relatorio -->
    <section class="secaoRelatorio">
        <div id="cartaoRelatorio">
            <span id="texto">RELATÓRIO</span>
            <div>
                <span id="subTexto">10/02/2025</span>
            </div>
        </div>
        <div class="tempoMedioBloco">
            <span id="subTexto1">Tempo médio gasto de viagem:</span>
            <span id="subTexto2">2h34m</span>
        </div>
        <div class="linhaRelatorio">
            <span id="subTexto3"><strong>Média de combustível gasto: </strong></span>
            <div>
                <div class="graficoCombustivel">
                    <span class="litros">415L</span>
                    <div id="chart_div" style="width: 600px; height: 110px;"></div>  
                </div>
            </div>

            <div>
                <span id="subTexto3"><strong>Advertências:</strong></span>
                <span id="subTexto3">● Excesso de velocidade</span>
                <span id="subTexto3">● Descuido na comunicação com a central de controle</span>
            </div>
            <div>
                <span id="subTexto3"><strong>Tempo de empresa: </strong>  1 anos, 9 meses e 27 dias.</span>
            </div>
            <div>
                <span id="subTexto3"><strong>Quantidades de viagens: </strong>  8.916 viagens.</span>
            </div>
        </div>
    </section>   
</body>
</html>